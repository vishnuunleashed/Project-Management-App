...
Future<void> showUOMListDialog(
    BuildContext context, {
      required List<UomModel> uomList,
      required void Function(String name) onForward,
      String title = 'Owners list',
    }) {
  final theme = Theme.of(context);
  List<UomModel> filteredNames = List.from(uomList);
  final TextEditingController searchController = TextEditingController();

  void filterList(String query) {
    filteredNames = uomList
        .where((uoms) =>
    uoms.uomDescription
        .toString()
        .toLowerCase()
        .contains(query.toLowerCase()) ||
        uoms.uomDescription!.toLowerCase().contains(query.toLowerCase()))
        .toList();
  }
                  ...
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return _NameTile(
                          name: name.uomDescription,
                          onForward: () => onForward(name.uomDescription),
                        );
                      },
                    ),
                  ),
