...
class ImageGridScreen extends StatelessWidget {
  final ProjectScheduleProvider projectScheduleProvider;
  const ImageGridScreen({
    super.key,
    required this.projectScheduleProvider
  ...
  Widget build(BuildContext context) {
    return BaseView<ImageGridProvider>(
      ...
      builder:(context,provider,ref) {
        final scheduleProvider = projectScheduleProvider;
        return Padding(
        padding: const EdgeInsets.all(12),
        child: GridView.builder(
          controller: provider.gridScrollController,
          itemCount: scheduleProvider.attachmentUrl.length, // show ALL images
          ...
          itemBuilder: (context, index) {
            AttachmentModel url = scheduleProvider.attachmentUrl[index];
            return GestureDetector(
              onTap: () {

                _openImageViewer(context, scheduleProvider, index);
              },
  ...
  static Future<void> _openImageViewer(BuildContext context, ProjectScheduleProvider provider, int initialIndex) async {
    try {
