...
class ServiceSupportMainWidget extends StatefulWidget {
  const ServiceSupportMainWidget({super.key});

  @override
  State<ServiceSupportMainWidget> createState() => _ServiceSupportMainWidgetWidgetState();
}

class _ServiceSupportMainWidgetWidgetState extends State<ServiceSupportMainWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    ViewServiceSupportRequestProvider _viewServiceSupportRequestProvider = container.read(viewServiceSupportRequestProvider);
    return WillPopScope(
      onWillPop: ()async{
        final hasRemarks = _viewServiceSupportRequestProvider.remarksController.text.isNotEmpty
        && _viewServiceSupportRequestProvider.supportListData?.requeststatus != "Closed";
        if (hasRemarks) {
      ...
      child: RefreshIndicator(
        onRefresh: ()async{
          _viewServiceSupportRequestProvider.fillSupportRequestDetails();
        },
        child: Scaffold(
          appBar: CustomAppBar(
            shadowNeeded: true,
                    ...
                    return ThreeDotMenu(
                      items: [
                        MenuItemModel(
                          ...
                          onTap: () {
                            _viewServiceSupportRequestProvider.changePage(1);
                          },
                        ),
                                  ...
                                  children: [
                                   provider.scheduleTaskDtlJson.isEmpty
                                    ...
                                    SupportDetailCommonHeader(
                                        projectName: provider.supportListData?.reftransaction.toString(),
  ...
  closeConfirmDialog({required BuildContext context,
    required ViewServiceSupportRequestProvider provider,
              ...
              Expanded(
                  child: BaseElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      // GoRouter.of(context).pop();
                      GoRouter.of(context).pushNamed(
                       AppRoutes.successServiceLoaderMySupport,
                        extra: {
                          'provider': provider,
                          'transNo': "${provider.supportListData?.transNo}",
                          'actionType' : actionType,
                          'title': (actionType == 'Forward Support') ? "Support request successfully forwarded to ${provider.selectedEmployee?.name}"
                              : (actionType == "Reassign Support") ? "Support request successfully reassigned to ${provider.selectedEmployee?.name}"
  ...
  void _submitForm(BuildContext context,ViewServiceSupportRequestProvider provider, WidgetRef ref,String status) {
    if(status == 'Submit'){
