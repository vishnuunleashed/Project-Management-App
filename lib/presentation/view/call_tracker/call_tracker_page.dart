...
import 'package:interior_design/presentation/view/call_tracker/widgets/call_track_filter_bottom_sheet.dart';
...
class CallTrackerPage extends ConsumerStatefulWidget {
  final bool isFromDashboard;
  const CallTrackerPage({super.key, this.isFromDashboard = false});

  @override
  ConsumerState<CallTrackerPage> createState() => _CallTrackerPageState();
}

class _CallTrackerPageState extends ConsumerState<CallTrackerPage> {



  @override
  Widget build(BuildContext context) {
    final provider = ref.read(callTrackerProvider);
    final theme = Theme.of(context);

    return BaseView<CallTrackerProvider>(
      initState:  (context, provider, ref){},
      provider: callTrackerProvider,
      virtualFloatingActionButton:(ref.watch(callTrackerProvider).statusFromGraph.isEmpty) ? Padding(
        ...
        child: Row(
          children: [
            FloatingActionButton(
                ...
                onPressed: () {
                  showModalBottomSheet(
                    ...
                    builder: (context) =>  CallTrackFilterBottomSheet(provider: provider),
                  );
                }
            ),
            SizedBox(width: 4,),
            FloatingActionButton(
                ...
                heroTag: "All Support",
                mini: true,
                child: Icon(Icons.support_agent,color: bayaInfraWhiteColor,),
                onPressed: () {
                //   [log] Request query :
                //   {action: CALL_TRACKER, Columns: [{Key: Start, Value: 0},
                // {Key: Limit, Value: 20}, {Key: Flag, Value: AGAINST},
                // {Key: SiteName, Value: Celestica}, {Key: Status, Value: OPENED},
                // {Key: LogStatus, Value: ASSIGNED}, {Key: DelayedYN, Value: N}]}
                  GoRouter.of(context)
                      .pushNamed(AppRoutes.serviceSupportRequestSiteWiseScreen, extra: {
                    "isFromCallTrackerALLSupport": true,
            ...
            FloatingActionButton(
                ...
                heroTag: "Dashboard",
                mini: true,
                child: Icon(Icons.dashboard_outlined,color: bayaInfraWhiteColor,),
                onPressed: (){
                  GoRouter.of(context).pushNamed(
                    AppRoutes.serviceTicketDashboardScreen,
                  );
                }
            ),
            SizedBox(width: 4,),
            Visibility(
               visible: provider.isProjectCoordinator || provider.isSuperUser,
              child: Row(
                children: [
                  FloatingActionButton(
                      backgroundColor: Theme.of(context).primaryColor,
                      ...
                      child: Icon(Icons.add,color: bayaInfraWhiteColor,),
                      onPressed: (){
                        GoRouter.of(context)
                            .pushNamed(AppRoutes.addServiceRequestScreen)
                            .then((_) {
                        });
                      }
                      ),
            ...
            FloatingActionButton(
              ...
              heroTag: "All Filter",
                  child: Icon(Icons.compare_arrows,color: bayaInfraWhiteColor,),
                  onPressed: (){
                  ...
                  FloatingActionButton(
                      ...
                      onPressed: () {
                        showModalBottomSheet(
                          ...
                          isScrollControlled: true,
                          backgroundColor: Colors.transparent,
                          builder: (context) =>  ServiceDashBoardFilterBottomSheet(callTrackerProvider: provider,dashboardProvider: dashboardProvider,),
                        );
                      }
                  ),
                ],
              ...
              child: SizedBox(
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      EmptyListView(
                          emptyText: "No pending service tickets found.",),
                        ...
                        children: [
                          ...
                          TextSpan(
                            text: ref
                                ...
                                .isEmpty
                                ? ''
                                : "(${ref.watch(callTrackerProvider).tickets.first.totalRecords})",
              ...
              child: RefreshIndicator(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Padding(
                      padding: EdgeInsets.only(bottom: index == (provider.tickets.length - 1)?86:0),
                      child: TicketCard(
                        ticket: ticket,
                        provider: provider,
...
class ServiceDashBoardFilterBottomSheet extends StatelessWidget {
  final CallTrackerProvider callTrackerProvider; //  keep to call fetchDashboardData
  ...
  const ServiceDashBoardFilterBottomSheet({
    super.key,
    required this.callTrackerProvider,
                      ...
                      children: [
                        Expanded(
                          child: BaseElevatedButton(
                            onPressed: () {
                              provider.clearFilters();
                              callTrackerProvider.loadTicketsFromGraph();
