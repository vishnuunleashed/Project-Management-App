...
class Eraser {
  ...
  /// Clears all notifications by Id
   static Future<void> clearAppNotificationsById(int id) async {
    try {
      await _channel
          .invokeMethod('clearAppNotificationsById', <String, dynamic>{
        'id': id,
      });
    } on PlatformException catch (e) {
      print(
          'Failed to clear app notifications for id ($id) for following reason: ${e.message}');
    }
  }
